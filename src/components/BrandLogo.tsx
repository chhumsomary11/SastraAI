...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  const iconSize =
    size === 'sm' ? 'w-7 h-7 text-sm rounded-lg' : size === 'lg' ? 'w-11 h-11 text-xl rounded-2xl' : 'w-9 h-9 text-lg rounded-xl';
  const textSize =
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 select-none ${className}`}>
      ...
      {/* Brand Text */}
      <div className={`font-bold tracking-tight ${textSize} font-['Plus_Jakarta_Sans',sans-serif] flex items-center gap-1`}>
